...
APP_NOPS_K8S_AGENT_CLUSTER_ARN="arn:aws:eks:us-west-2:844856862745:cluster/uat-container-rightsizing" # You can find this on your EKS dashboard on AWS
...
APP_AWS_S3_BUCKET="nops-container-cost-844856862745"
...
PROMETHEUS_CONFIG_URL_DEBUG="https://raw.githubusercontent.com/nops-io/nops-k8s-agent/master/easy-install/prometheus-ksm-debug-nvidia.yaml"
...
if [[ "$USE_CUSTOM_REGISTRY" == "true" ]]; then
    if [[ "$DEBUG_MODE" == "true" ]]; then
        ...
        helm upgrade --install nops-prometheus prometheus --repo https://prometheus-community.github.io/helm-charts \
            --namespace nops-prometheus-system --create-namespace -f $PROMETHEUS_CONFIG_URL_DEBUG \
    ...
    if [[ "$DEBUG_MODE" == "true" ]]; then
        # Installing nops-Prometheus
        # kubectl create namespace gpu-metrics
        # helm repo add nvdp https://nvidia.github.io/k8s-device-plugin
        # helm repo update
        # helm install --namespace gpu-metrics gpu-metrics-exporter nvdp/nvidia-dcgm-exporter

        helm upgrade --install nops-prometheus prometheus --repo https://prometheus-community.github.io/helm-charts  --namespace nops-prometheus-system --create-namespace -f ./prometheus-ksm-debug-nvidia.yaml || { echo "Error: Failed to install Prometheus"; exit 1; }

        helm upgrade -i nops-prometheus-crds prometheus-community/prometheus-operator-crds 

        helm upgrade -i nops-gpu-metrics-exporter gpu-helm-charts/dcgm-exporter -f ./cdr-operator.yaml 
